

export function Home() {
  return (
    <div>
      <h2>🏆 Premier League title race (as of March 1, 2026)</h2>
      <p>
        Arsenal are currently top of the Premier League table, leading the standings with around 61 points from about 28 matches played.

        Manchester City are in second place,
        trailing by just 2 points after their recent 1-0 win over Leeds United — closing the gap significantly in the title race.
        Manchester City have been in strong form, extending their unbeaten run and keeping up pressure on Arsenal even without top scorer Erling Haaland.
        Arsenal have had impressive results too, including a big North London derby win and strong consistency at the top.
        Aside from the league, the two clubs are also set to meet in cup competitions:

        They are scheduled to play in the 2026 EFL Cup final on 22 March 2026 at Wembley Stadium — a direct title showdown in a major domestic cup.
        Both sides have key league fixtures left and will look to maintain momentum.

        The race could go down to the final weeks of the season given the small gap.

        Head-to-head matches — especially the league game on 18 April 2026 and the EFL Cup final — will be pivotal in deciding silverware.
      </p>
      <h2>
        ⚽2025/26 Premier League performance
      </h2>
      <p>
        The Premier League is widely considered the most competitive domestic football league in the world.
        📊 Current Standings (top of the table)

        Arsenal – leading the title race with a healthy points advantage

        Manchester City – close challengers, narrowing the gap lately

        Aston Villa – third and surprising strong performers

        Manchester United – in contention for European places
        5–6. Chelsea & Liverpool – around mid-top, with Liverpool still eyeing a top-four spot.
      </p>
      <h2>
        🏆 Title Race & Form
      </h2>
      <p>
        Arsenal have been consistent and sit top, holding a few points lead.

        Manchester City have recently gained momentum and are within striking distance.

        Liverpool remain in contention but are trailing the top two and fighting for Champions League qualification.
        Recent wins, like a 5-2 surge vs West Ham, helped them stay in touch.
        The league remains highly competitive, with multiple clubs vying for top-four spots.

        Matches have featured high-scoring games and surprise results, reflecting strong depth beyond the traditional “big six.”
        The 2025-26 Premier League season is shaping up as a tight title race primarily between Arsenal and Manchester City, while several traditional powers and challengers aim for European qualification.
        At the other end, the relegation battle is also fiercely contested.
      </p>
      <h2>
        <a href="https://business.globbalnews.com">Join our business pages for online sections</a>
      </h2>
      <h2>
        2025/26 🏆 UEFA Champions League
      </h2>
      <p>
        The Champions League is the most prestigious club competition in world football.
        It differs from the Leagues in structure and intensity.
        Elite-only competition: Features the best teams from Europe’s top leagues.

        Knockout drama: Two-legged ties and finals create high-pressure, high-stakes football.

        Tactical variety: Clashes between different football philosophies (e.g., Spanish possession, German pressing, Italian defensive structure).

        Global prestige: Winning it defines legacies of clubs and players.
      </p>
      <h2>
        Qualification wasn’t dominant
      </h2>
      <p>
        Real Madrid did progress to the Round of 16, but they finished 9th in the league phase under the new UCL format and had to go through a play-off versus SL Benfica rather than qualifying directly like champions usually do — a sign they weren’t as strong early on.
      </p>
      <h2>
        <a href="https://globbalnews.com">Breaking News</a>
      </h2>
      <h2>
        <a href="https://shop.globbalnews.com">Contact us for business informations</a>
      </h2>
    </div>
  );
}
